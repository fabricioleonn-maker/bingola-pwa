...
const GlobalMusicHeader: React.FC<Props> = ({ currentScreen }) => {
    ...
    return (
        <div className={`fixed top-0 left-0 right-0 z-[200] flex flex-col items-center transition-all duration-500 ease-in-out ${isExpanded ? 'bg-background-dark/95 backdrop-blur-xl border-b border-white/10 pb-6' : 'h-4 hover:h-10 group bg-white/[0.02] hover:bg-white/[0.08]'}`}>
            ...
            >
                ...
                {!isExpanded && (
                    <div className="absolute right-4 flex items-center gap-3">
                        ...
                        {/* Beat Animation - Ghostly when not hovered */}
                        {isPlaying && !isMuted && (
